...
public class Lab02app {
    ...
    public static void main(String[] args) throws InterruptedException {
        // whats it have to have access to .. carts list, products, and barcodes 
        TreeMap<String, String> barcodes = new TreeMap<String, String>();
        ProductList pt = new ProductList();
        CartsList ct = new CartsList();
        // Barcodes database could be moved to its own class as well -- TBD
        initBarCodes(barcodes);

        int MaxNumThreads = 100; // maximum number of threads 
        // note: if we reach this many threads we can keep looping or sleep a bit
        // ThreadIndex is just used for debugging
        int ThreadIndex = 0;
        for (ArrayList<String> lists: ct.cl) { // ct = cart list class , cl = actual data
            int activeThreads = Thread.activeCount();
            // if we have more than MaxNumThreads active threads we continue
            // looping (maybe with sleep)
            //System.out.println("Active Threads = " + activeThreads);
            if (activeThreads < MaxNumThreads) {
                System.out.println("Spawn Thread " + ThreadIndex);
                CartThread t = new CartThread(ThreadIndex, lists, pt.pls, barcodes);
                t.start();
                ThreadIndex++;                                
            } else {
                System.out.println("Max Threads reached: activeThreads = " + activeThreads);
                // sleep a bit here if necessary? 
            }
        }
    }   
